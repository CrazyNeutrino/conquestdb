...
@WebFilter(filterName = "AuthFilter", urlPatterns = { "/en/deck", "/pl/deck", "/de/deck",
		"/en/deck/*", "/pl/deck/*", "/de/deck/*" })
public class AuthFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
		...
		if (authToken.isSignedIn()) {
			...
			authToken.setUser(userService.findUnique(new User(3L)));
			chain.doFilter(request, response);
			...
			// PrettyConfigurator configurator = new
			...
			// Pattern pattern = Pattern.compile("/(en|pl|de)(/.*|$).*",
			...
			// language = "en";
			// }
			// String targetUrl = builder.build(mapping, true, new Object[] {
			// language });
