...
public enum Faction {
	...
	ASTRA_MILITARUM, SPACE_MARINES, TAU, ELDAR, DARK_ELDAR, CHAOS, ORK, NEUTRAL;
	...
	public Faction[] allies() {
		Faction[] allies;
		if (this == NEUTRAL) {
			allies = new Faction[0];
		} else {
			allies = new Faction[3];
	...
	public Faction[] alliance() {
		Faction[] alliance;
		if (this == NEUTRAL) {
			alliance = new Faction[0];
		} else {
			alliance = new Faction[4];
