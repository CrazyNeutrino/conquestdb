...
public class DeckCommentDao extends JpaDaoAbstract<DeckComment, DeckCommentQuery> {
	...
	@Override
	protected void fetchRelatedEntities(Query<DeckComment> query, Root<DeckComment> root) {
		root.fetch(DeckComment_.deck);
		root.fetch(DeckComment_.user);
	}
	...
	protected List<Predicate> buildCustomCriteria(DeckCommentQuery query, Root<DeckComment> root) {
		...
		if (deck != null) {
			...
			if (deck.getSnapshotPublic() != null) {
				predicates.add(cb.equal(deckPath.get(Deck_.snapshotPublic), deck.getSnapshotPublic()));
			}
		}
